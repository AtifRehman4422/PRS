...
class ActiveAdsPage extends ConsumerStatefulWidget {
  const ActiveAdsPage({super.key});

  @override
  ConsumerState<ActiveAdsPage> createState() => _ActiveAdsPageState();
}

class _ActiveAdsPageState extends ConsumerState<ActiveAdsPage> {
  ...
  Future<void> _load() async {
    ...
    final activeOnly = models.where((l) => (l.status ?? 'active').toLowerCase() == 'active').toList();
    if (_selectedType == null && activeOnly.isNotEmpty) {
      _selectedType = activeOnly.first.propertyType;
    }
    _allAds = activeOnly;
                      ...
                      : _ads.isEmpty
                          ? Center(
                              child: Text(
                                _selectedType == null
                                    ? 'No active ads.'
                                    : 'No $_selectedType ads in active.',
                                style: TextStyle(color: colorScheme.onSurface.withValues(alpha: 0.7), fontSize: 14),
                              ),
                            )
                          : ListView.builder(
